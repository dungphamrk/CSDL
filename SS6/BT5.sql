...
SELECT u.user_id,u.username,Count(p.user_id) as total_posts,SUM(p.likes_count) as total_likes
FROM Users u
JOIN Posts p on p.user_id = u.user_id 
GROUP BY u.user_id,u.username
Having total_likes >20;

SELECT u.user_id,u.username,p.post_id,p.content, MAX(likes_count) as max_likes
FROM Users u
JOIN Posts p on p.user_id = u.user_id 
...
SELECT u.user_id,u.username,p.post_id,p.content, MIN(likes_count) as min_likes
FROM Users u
JOIN Posts p on p.user_id = u.user_id 
GROUP BY u.user_id,u.username,p.post_id,p.content
Having min_likes <15;
